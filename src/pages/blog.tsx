...
export default function Blog() {
  ...
  return (
    <main className="blog-container flex items-center justify-center pt-32 pb-4">
      <div className="flex-1 flex flex-col items-center gap-8 min-h-64">
        ...
        <section className="blog-content flex flex-col items-center gap-4">
          {status === "loading" && <p className="post-status-panel w-[800px] max-w-[90vw]">Loading posts...</p>}
          {status === "error" && <p className="post-status-panel w-[800px] max-w-[90vw]">Posts are unavailable right now.</p>}
          {status === "ready" && posts.length === 0 && <p className="post-status-panel w-[800px] max-w-[90vw]">Posts coming soon.</p>}
          {posts.map((post) => (
            <Link key={post._id} to={`/blog/${post.slug}`} className="post-preview-link w-[800px] max-w-[90vw] block">
